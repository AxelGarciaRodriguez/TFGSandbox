...
from literals import IMAGE_BASE_PATH, NOT_FOUND_IMAGE_NAME, IMAGE_KINECT_SAVE_PATH, CALIBRATE_PATTERN_IMAGES, \
    KinectFrames, PATTERN_IMAGE_NAME
...
def calibrate_kinect(kinect, principal_screen, projector_screen, use_previous_images, save_new_images):
    ...
    pattern_image = ImageObject(
        image_absolute_path=generate_relative_path([IMAGE_BASE_PATH, PATTERN_IMAGE_NAME])).image

    # Initiate process
    projector_window = projector_screen.create_window(window_name="FullScreen Projector", image=background_color_image,
                                                      fullscreen=True)

    app = instantiate_calibrate_interface(kinect=kinect, previous_images=previous_images,
                                          not_found_image=not_found_image, pattern_image=pattern_image,
                                          projector_screen=projector_screen, projector_window=projector_window)
